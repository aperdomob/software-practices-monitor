...
const hasTeamRule = (teamName: string, permission: string, level: Level) => ({
  ...
  expression: (repository: Repository) => {
    const team = repository.settings.manageAccess.teams.find(
      current => current.name === teamName,
    );
  ...
  children: [
    {
      ...
      expression: (team: Collaborator) =>
        assert.equal(
          team.permission,
          permission,
          `the team should have permission of ${permission} and has ${team.permission}`,
        ),
      level: level,
    },
  ],
...
const hasProtectionRule = ({
  ...
  const children: Rule<BranchProtectionRules>[] = [
        ...
        ? [
            {
              ...
              expression: rule =>
                assert.isAtLeast(
                  ...
                  codeReviewers,
                  `require minimal ${codeReviewers} reviwers`,
                ),
              level: Level.ERROR,
            },
          ]
    ...
    },
    {
      ...
      expression: rule =>
        assert.equal(
          rule.dismissStale,
          dismissStale,
          `dissmiss stale is ${dismissStale ? 'disabled' : 'enabled'}`,
        ),
      level: Level.WARNING,
    },
    {
      ...
      expression: rule =>
        assert.equal(
          rule.requireReviewFromCodeOwners,
          reviewOwners,
          `require review from Code Owners is ${
            reviewOwners ? 'disabled' : 'enabled'
          }`,
        ),
      level: Level.ERROR,
    },
        ...
        ? [
            {
              ...
              expression: rule => {
                const hook = rule.statusChecks.find(
                  query => query === 'continuous-integration/jenkins/branch',
                );

                assert.isNotNull(
                  hook,
                  'continuous-integration/jenkins/branch is required',
                );
              },
              level: Level.ERROR,
            },
            {
              ...
              expression: rule =>
                assert.equal(
                  rule.requireBranchesToBeUpToDataBeforeMerging,
                  true,
    ...
    },
    {
      ...
      expression: rule =>
        assert.isTrue(
          ...
          'required Signed Commit is mandatory',
        ),
      level: Level.CRITICAL,
    },
    {
    ...
    },
    {
      ...
      expression: rule =>
        assert.isTrue(
          rule.includeAdministrators,
          'Include Administrator is disable',
        ),
      level: Level.CRITICAL,
    },
  ];
  ...
  return {
    ...
    expression: (repository: Repository) => {
      const rule = repository.settings.branches.protectionRules.find(
...
export const GithubRules: Rule<Repository>[] = [
  {
    ...
    expression: repository =>
      assert.isNotNull(
        ...
        'the repository does not have a description',
      ),
    level: Level.WARNING,
  },
  {
    ...
    expression: repository =>
      assert.isTrue(repository.isPrivate, 'the repository is not private'),
    level: Level.CRITICAL,
  },
  {
    name: 'pages-disabled',
    expression: repository =>
      assert.isFalse(
        repository.settings.general.githubPages,
        'the repository has pages',
      ),
    level: Level.CRITICAL,
  },
  {
  ...
  },
  {
    ...
    expression: repository =>
      assert.isTrue(
        repository.settings.general.dataServices.securityAlerts,
        'the repository does not have configured security alerts',
      ),
    level: Level.CRITICAL,
  },
  {
    ...
    expression: repository =>
      assert.isTrue(
        repository.settings.general.mergeButton.allowMergeCommits,
        'the commit buttons is disabled',
      ),
    level: Level.ERROR,
  },
  {
    ...
    expression: repository =>
      assert.isFalse(
        repository.settings.general.mergeButton.allowSquashMerging,
        'the squash buttons is enable',
      ),
    level: Level.CRITICAL,
  },
  {
  ...
  hasTeamRule('cc-leads', 'admin', Level.WARNING),
  {
    ...
    expression: repository => {
      ...
      assert.equal(
        0,
        total,
        `the repository has configured ${total} people directly and should have 0`,
      );
    },
    level: Level.WARNING,
  },
  ...
  protectionAllBranch(),
  {
    ...
    expression: repository =>
      assert.isAtMost(
        ...
        0,
        'the repository has vulnerabilities',
      ),
    level: Level.CRITICAL,
  },
];
